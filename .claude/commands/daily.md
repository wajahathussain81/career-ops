---
description: Morning briefing — sync remote nightly work, email-check results, pipeline status, today's plan
---
...
## 1. Sync the remote hub (pull-based — always do this first)
- Run `git fetch hub-remote`.
- If behind: `git merge --ff-only hub-remote/main`; if the branches diverged, do a regular merge with message "merge hub nightly runs".
