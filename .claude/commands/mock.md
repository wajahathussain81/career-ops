...
Argument: a company or prep-folder fragment (e.g. `acme`, `globex`).
...
## Hard rules
- Never put invented claims in the candidate's mouth — suggested phrasings must trace to `cv.md`, the prep doc, or the story bank. Respect every ownership boundary stated in the prep doc for the candidate’s specific projects and employers.
...
## Troubleshooting
- Silent transcript: mic device issue — `talk` records from "MacBook Air Microphone" by default; `TALK_MIC` env overrides (e.g. `TALK_MIC="<your-microphone-device>"`). Device `:0` (Microsoft Teams Audio) is a silent virtual device — never use it. Run `~/bin/voice-mock-setup.sh` for a health check.
