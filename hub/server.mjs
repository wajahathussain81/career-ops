  ...
  return [
    ...
    },
    {
      ...
      handler: async (_req, res) => send(res, 200, renderPage({
        title: 'Packages',
        active: '/packages',
        body: renderPackages(listPendingPackages(app.root)),
      })),
    },
    {
      method: 'GET',
    ...
    },
    {
      ...
      handler: async (req, res) => {
        ...
        if (body.state === 'Applied') {
          execFile('node', ['followup-seed.mjs', String(body.num)], { cwd: app.root },
            (error, _stdout, stderr) => {
              if (error) console.error(`Follow-up seed failed for #${body.num}:`, stderr || error.message);
            });
        }
        sendJson(res, { ok: true });
      },
