...
#
# Prereqs: `pve-host` host alias in ~/.ssh/config (key auth), branch
# pushed to the NAS remote the container pulls from (`git push hub-remote`).

set -euo pipefail

PVE_HOST="${PVE_HOST:-pve-host}"
CT_ID="${CT_ID:-100}"
BRANCH="${BRANCH:-main}"
