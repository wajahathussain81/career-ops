...
import { mdToHtml, safeUrl } from '../lib/md.mjs';
...
function renderPeople(contacts, followUps) {
  ...
  ${contacts.length ? `<div class="table-scroll"><table class="data"><thead><tr><th>Name</th><th>Type</th><th>Title</th><th>Links</th></tr></thead><tbody>${contactRows}</tbody></table></div>` : '<p>No contacts recorded.</p>'}
  <h2>Follow-ups</h2>
  ${followUps.length ? `<div class="table-scroll"><table class="data"><thead><tr><th>Date</th><th>Channel</th><th>Contact</th><th>Notes</th></tr></thead><tbody>${followUpRows}</tbody></table></div>` : '<p>No follow-ups recorded.</p>'}`;
}
...
function renderResume(resume, num) {
  ...
  const src = `/files/resume/${escapeHtml(num)}`;
  return `<a href="${src}" target="_blank" rel="noopener">Open resume PDF ↗</a>
  <embed src="${src}" type="application/pdf" width="100%" height="800">`;
}
...
export function renderApplicationDetail(detail) {
  ...
  <div class="tabs" role="tablist">${tabs.map((tab, index) => `<button class="tab" id="app-tab-${index}" type="button" role="tab" aria-selected="${index === 0}" aria-controls="app-pane-${index}" tabindex="${index === 0 ? '0' : '-1'}" data-tab-target="app-pane-${index}">${tab}</button>`).join('')}</div>
  ${panes.map((pane, index) => `<section id="app-pane-${index}" role="tabpanel" aria-labelledby="app-tab-${index}" data-tab-pane${index === 0 ? '' : ' hidden'}>${pane}</section>`).join('')}
</main>`;
}
